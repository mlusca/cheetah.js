import { Context, CONTROLLER_ROUTES, Metadata, ROUTE_PARAM, TokenRoute } from '@cheetah.js/core';
...
const createMethodDecorator = (methodType: string) => {
    return (path: string = ''): MethodDecorator => {
        return (target, propertyKey) => {
            ...
            routes.push({ method: methodType, path, methodName: propertyKey.toString(), middlewares: [] });
            Metadata.set(CONTROLLER_ROUTES, routes, target)
        }
    }
}

export function createParamDecorator(func: (context: Context, data?: any) => any) {
   return (data?: any): ParameterDecorator =>
     (target, propertyKey, parameterIndex) => {
       const existingArgs:Record<number, any> = Metadata.get(ROUTE_PARAM, target.constructor, propertyKey) || {};
...
export const Body = createParamDecorator((context: Context, data: string) =>  data ? context.body[data] : (context.body || {}));
